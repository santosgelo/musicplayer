...
/**
 * Created by Mark Angelo Santos on 7/29/2015.
 */
public class SeekArc extends View {
    ...
    private void initAttributes(TypedArray attrArray) {
        mCircleXRadius = attrArray.getFloat(R.styleable.SeekArc_circle_x_radius, DEFAULT_CIRCLE_X_RADIUS) * DPTOPX_SCALE;
        mCircleYRadius = attrArray.getFloat(R.styleable.SeekArc_circle_y_radius, DEFAULT_CIRCLE_Y_RADIUS) * DPTOPX_SCALE;
        mPointerRadius = attrArray.getFloat(R.styleable.SeekArc_pointer_radius, DEFAULT_POINTER_RADIUS) * DPTOPX_SCALE;
        mPointerHaloWidth = attrArray.getFloat(R.styleable.SeekArc_pointer_halo_width, DEFAULT_POINTER_HALO_WIDTH) * DPTOPX_SCALE;
        mPointerHaloBorderWidth = attrArray.getFloat(R.styleable.SeekArc_pointer_halo_border_width, DEFAULT_POINTER_HALO_BORDER_WIDTH) * DPTOPX_SCALE;
        mCircleStrokeWidth = attrArray.getFloat(R.styleable.SeekArc_circle_stroke_width, DEFAULT_CIRCLE_STROKE_WIDTH) * DPTOPX_SCALE;

        String tempColor = attrArray.getString(R.styleable.SeekArc_pointer_color);
        if (tempColor != null) {
            try {
        ...
        tempColor = attrArray.getString(R.styleable.SeekArc_pointer_halo_color);
        if (tempColor != null) {
            try {
        ...
        tempColor = attrArray.getString(R.styleable.SeekArc_pointer_halo_color_ontouch);
        if (tempColor != null) {
            try {
        ...
        tempColor = attrArray.getString(R.styleable.SeekArc_circle_color);
        if (tempColor != null) {
            try {
        ...
        tempColor = attrArray.getString(R.styleable.SeekArc_circle_progress_color);
        if (tempColor != null) {
            try {
        ...
        tempColor = attrArray.getString(R.styleable.SeekArc_circle_fill);
        if (tempColor != null) {
            try {
        ...
        mPointerAlphaOnTouch = attrArray.getInt(R.styleable.SeekArc_pointer_alpha_ontouch, DEFAULT_POINTER_ALPHA_ONTOUCH);
        if (mPointerAlphaOnTouch > 255 || mPointerAlphaOnTouch < 0) {
            mPointerAlphaOnTouch = DEFAULT_POINTER_ALPHA_ONTOUCH;
        }

        mMax = attrArray.getInt(R.styleable.SeekArc_max, DEFAULT_MAX);
        mProgress = attrArray.getInt(R.styleable.SeekArc_progress, DEFAULT_PROGRESS);
        mCustomRadii = attrArray.getBoolean(R.styleable.SeekArc_use_custom_radii, DEFAULT_USE_CUSTOM_RADII);
        mMaintainEqualCircle = attrArray.getBoolean(R.styleable.SeekArc_maintain_equal_circle, DEFAULT_MAINTAIN_EQUAL_CIRCLE);
        mMoveOutsideCircle = attrArray.getBoolean(R.styleable.SeekArc_move_outside_circle, DEFAULT_MOVE_OUTSIDE_CIRCLE);
        lockEnabled = attrArray.getBoolean(R.styleable.SeekArc_lock_enabled, DEFAULT_LOCK_ENABLED);

        // Modulo 360 right now to avoid constant conversion
        mStartAngle = ((360f + (attrArray.getFloat((R.styleable.SeekArc_start_angle), DEFAULT_START_ANGLE) % 360f)) % 360f);
        mEndAngle = ((360f + (attrArray.getFloat((R.styleable.SeekArc_end_angle), DEFAULT_END_ANGLE) % 360f)) % 360f);
    ...
    private void init(AttributeSet attrs, int defStyle) {
        final TypedArray attrArray = getContext().obtainStyledAttributes(attrs, R.styleable.SeekArc, defStyle, 0);
    ...
    public interface OnCircularSeekBarChangeListener {
        ...
        void onProgressChanged(SeekArc circularSeekBar, int progress, boolean fromUser);

        void onStopTrackingTouch(SeekArc seekBar);

        void onStartTrackingTouch(SeekArc seekBar);
    }
